...
def validate_materials(spec: dict[str, Any], errors: list[str], warnings: list[str]) -> set[str]:
    ...
    for index, material in enumerate(spec.get("materials", [])):
        ...
        for field in ("normal", "bump", "displacement", "wear", "dirt", "emissive"):
            value = material.get(field)
            if value is not None and not isinstance(value, dict):
                errors.append(f"material {material_id!r} {field} must be an object")
        for field in ("clearcoat", "clearcoatRoughness", "transmission", "opacity"):
        ...
        if isinstance(emissive, dict):
            emissive_color = emissive.get("color")
            if emissive_color is not None and not (
                isinstance(emissive_color, str)
                and emissive_color.startswith("#")
                and len(emissive_color) in {4, 7}
            ):
                errors.append(f"material {material_id!r} emissive.color should be #RGB or #RRGGBB")
            emissive_intensity = emissive.get("intensity")
            if emissive_intensity is not None and (
                not is_number(emissive_intensity) or float(emissive_intensity) < 0
            ):
                errors.append(f"material {material_id!r} emissive.intensity must be non-negative")
...
def validate_visual_evidence_history(
    ...
    for index, item in enumerate(visual_history):
        ...
        for failure in visual_evidence_hash_failures(
            item,
            spec_path,
            require_local=False,
        ):
