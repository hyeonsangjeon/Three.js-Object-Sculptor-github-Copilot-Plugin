...
def dimension_values(component: dict[str, Any]) -> list[float]:
...
def visual_scale_vector(component: dict[str, Any], transform: dict[str, Any]) -> str:
    dimensions = dimension_values(component)
    ...
    if not (
        ...
        scale = [1.0, 1.0, 1.0]
    return vector(
        [dimensions[index] * float(scale[index]) for index in range(3)],
        [1.0, 1.0, 1.0],
    )
...
def generate(spec: dict[str, Any], pass_id: str) -> str:
    ...
    for index, component in enumerate(components):
        ...
        uses_endpoint_geometry = primitive in ENDPOINT_PRIMITIVES
        ...
        if uses_endpoint_geometry:
            ...
            placement_lines = [
                ...
                "  } else {",
                f"    {component_var}.scale.set({visual_scale_vector(component, transform)});",
                "  }",
            ]
        else:
            geometry_lines = [
                f"  const {component_var}Geometry = {geometry_for(primitive)};",
            ]
            placement_lines = [
                f"  if ({endpoint_var}) {{",
                f"    {component_var}.position.set({vector(transform.get('position'), [0, 0, 0])});",
                f"    {component_var}.position.sub({endpoint_var}.start);",
                f"    {component_var}.rotation.set({vector(transform.get('rotation'), [0, 0, 0])});",
                "  }",
                f"  {component_var}.scale.set({visual_scale_vector(component, transform)});",
            ]
        lines.extend(
    ...
    sculpt_dna = spec.get("sculptDNA", {})
    variant_provenance = spec.get("variantProvenance")
    lines.extend(
        [
            "",
            "  root.userData.sculptRuntime = { nodes, meshes, sockets, colliders, destructionGroups } satisfies ProceduralModelRuntime;",
            f"  root.userData.lookDevTargets = {json_literal(look_dev_targets)};",
            f"  root.userData.sculptDNA = {json_literal(sculpt_dna)};",
            f"  root.userData.variantProvenance = {json_literal(variant_provenance)};",
